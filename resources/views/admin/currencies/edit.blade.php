...
<div class="container">
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h3 class="mb-0">@lang('Currencies')</h3>
            <a href="{{ route('admin.currencies.index') }}" class="btn btn-primary">@lang('List Currencies')</a>
        </div>
            ...
            <form action="{{ isset($currency) ? route('admin.currencies.update', $currency) : route('admin.currencies.store') }}"
                ...
                <div class="mb-3 form-check">
                    <input type="checkbox" name="is_default" class="form-check-input"
                        {{ (isset($currency) && $currency->is_default) ? 'checked' : '' }}>
                    <label class="form-check-label">{{ __('messages.default') }}</label>
                </div>
